...
import { Lock, Smartphone, User, ArrowRight } from 'lucide-react'
...
export default function Home() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            window.location.href = (profile?.role === 'admin' || email.includes('admin')) ? '/admin' : '/user'
        }
    }
        ...
        <main className="flex min-h-screen flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="glass-card w-full max-w-md p-8 rounded-2xl relative z-10 animate-fade-in">
                <div className="flex flex-col items-center text-center mb-8">
                    <div className="mb-6 animate-float"><Logo size="large" /></div>
                    <h1 className="text-4xl font-black bg-gradient-to-r from-sky-500 via-cyan-500 to-teal-500 bg-clip-text text-transparent">Fresh-Rider</h1>
                </div>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    <div className="space-y-4">
                        <div className="relative group">
                            ...
                            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                                className="glass-input w-full !pl-12 rounded-xl py-3 text-sm" placeholder={t('email')} />
                        </div>
                        <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            ...
                            <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)}
                                className="glass-input w-full !pl-12 rounded-xl py-3 text-sm" placeholder={t('password')} />
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                        <button type="submit" disabled={loading}
                            className="w-full flex justify-center items-center py-3 px-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-xl shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 group">
                            {loading
                                ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                                : <span className="flex items-center gap-2">Giri\u015f Yap <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></span>
                            }
                        </button>
                        <button type="button" onClick={() => router.push('/register')} disabled={loading}
                            ...
                            Hesab\u0131n\u0131z yok mu? <span className="text-brand-accent">Kay\u0131t Olun</span>
                        </button>
                    </div>
                </form>
...
fs.writeFileSync('src/app/page.tsx', loginPage, 'utf8');
console.log('Login page written OK');