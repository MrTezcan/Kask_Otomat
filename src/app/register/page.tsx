...
export default function Register() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        // Phone uniqueness check - only block if explicitly not unique (ignore RPC errors)
        ...
        // Let Supabase Auth handle email uniqueness natively
        ...
        <main className="flex min-h-screen flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="glass-card w-full max-w-md p-8 rounded-2xl relative z-10 animate-fade-in shadow-2xl shadow-black/50">
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                    ...
                    {error && (
                        <div className={"p-3 text-xs rounded-lg border backdrop-blur-md " + (error.includes('ba\u015far') ? 'text-emerald-300 bg-emerald-900/40 border-emerald-500/30' : 'text-red-300 bg-red-900/40 border-red-500/30')}>
                            {error}
                        </div>
                    )}
