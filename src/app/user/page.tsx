...
export default function UserDashboard() {
    ...
    useEffect(() => {
        if (!userId) return
        const fetchNotifications = async () => { const { data } = await supabase.from('notifications').select('*').or(`user_id.eq.${userId},user_id.is.null`).order('is_read', { ascending: true }).order('created_at', { ascending: false }).limit(20); if (data) setNotifications(data) }
        fetchNotifications()
