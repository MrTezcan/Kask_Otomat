...
export default function UserDashboard() {
    ...
    const handleQrPayment = async () => {
        if (!qrDeviceId) return alert('L�tfen makine kodunu girin.')
        const device = devices.find(d => d.id === qrDeviceId || d.name === qrDeviceId)
        if (!device) return alert('Ge�ersiz makine kodu veya cihaz bulunamad�.')
        if (device.status !== 'online') return alert('Bu makine �u anda hizmet veremiyor (�evrimd���/Bak�mda).')
        ...
        try {
            ...
            // Notification
            await supabase.from('notifications').insert({ user_id: userId, type: 'success', title: '�deme Ba�ar�l�', message: ${device.name} cihaz�nda  ? �deme yap�ld�. ��lem ba�l�yor... })

            alert('�deme ba�ar�l�! Makine �al��maya ba�l�yor.')
    ...
    const handlePasswordChange = async (e: React.FormEvent) => {
        e.preventDefault()
        if (newPassword.length < 6) return alert('�ifre en az 6 karakter olmal�d�r.')
        
        const { error } = await supabase.auth.updateUser({ password: newPassword })
        if (error) {
            alert('�ifre g�ncellenirken hata olu�tu: ' + error.message)
        } else {
            alert('�ifreniz ba�ar�yla g�ncellendi.')
            setNewPassword('')
        }
    }
                    ...
                    <div className="bg-white w-full sm:max-w-md sm:rounded-[2rem] rounded-t-[2rem] p-6 relative z-10 animate-fade-in-up">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                >
                                    <option value="">Se�iniz veya Okutunuz...</option>
                            ...
                            >
                                {paymentProcessing ? '��leniyor...' : '50 ? �de ve Ba�lat'}
                            </button>
                        </div>
                    </div>
                </div>
