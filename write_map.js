...
const kioskIcon = L.divIcon({ className: 'custom-icon', html: \<div style="background:linear-gradient(135deg,#10b981,#059669);width:32px;height:32px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2px solid white;box-shadow:2px 2px 10px rgba(16,185,129,.5);display:flex;align-items:center;justify-content:center"><div style="transform:rotate(45deg);font-weight:900;color:white;font-size:14px">FR</div></div>\, iconSize:[32,32], iconAnchor:[16,32], popupAnchor:[0,-32] })
const kioskOfflineIcon = L.divIcon({ className: 'custom-icon', html: \<div style="background:linear-gradient(135deg,#64748b,#475569);width:32px;height:32px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2px solid white;display:flex;align-items:center;justify-content:center"><div style="transform:rotate(45deg);color:white;font-weight:bold">X</div></div>\, iconSize:[32,32], iconAnchor:[16,32], popupAnchor:[0,-32] })
const kioskMaintenanceIcon = L.divIcon({ className: 'custom-icon', html: \<div style="background:linear-gradient(135deg,#f59e0b,#d97706);width:32px;height:32px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2px solid white;display:flex;align-items:center;justify-content:center"><div style="transform:rotate(45deg);color:white;font-weight:bold">!</div></div>\, iconSize:[32,32], iconAnchor:[16,32], popupAnchor:[0,-32] })
const nearestKioskIcon = L.divIcon({ className: 'custom-icon', html: \<div style="background:linear-gradient(135deg,#06b6d4,#3b82f6);width:44px;height:44px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 0 25px rgba(6,182,212,.6);display:flex;align-items:center;justify-content:center;animation:pulse 2s infinite"><div style="transform:rotate(45deg);font-weight:900;color:white;font-size:18px">*</div></div>\, iconSize:[44,44], iconAnchor:[22,44], popupAnchor:[0,-44] })
const userIcon = L.divIcon({ className: 'custom-icon', html: \<div style="background:#3b82f6;width:16px;height:16px;border-radius:50%;border:3px solid white;box-shadow:0 0 0 10px rgba(59,130,246,.2)"></div>\, iconSize:[16,16], iconAnchor:[8,8], popupAnchor:[0,-10] })
...
export default function KioskMap({ userLocation, kiosks }: { userLocation: [number,number], kiosks: any[] }) {
    ...
    const nearestKiosk = onlineKiosks.length > 0 ? onlineKiosks.reduce((p: any, c: any) => c.distance < p.distance ? c : p) : null
                        ...
                        <Marker key={kiosk.id} position={[lat, lng]} icon={getKioskIcon(kiosk.status || 'online', isNearest)}>
                            <Popup>
                                <div className="text-slate-800">
                                    ...
                                    <span className={\	ext-xs font-bold \\}>{statusInfo.text}</span>
...
;
fs.writeFileSync('src/components/KioskMap.tsx', code.trim(), 'utf8');
